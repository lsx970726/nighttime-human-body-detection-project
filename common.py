...
class SPPFCSPC(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        ...
        # Create an instance of the SimAM module
        self.simam = SimAM()

    def forward(self, x):
        # x = self.simam(x) # Apply the SimAM module
        x1 = self.cv4(self.cv3(self.cv1(x)))
...
class CAConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, contrast_factor=1.1):
    ...
    def forward(self, x):
        adjusted_x = x * self.contrast_factor
        return self.act(self.bn(self.conv(adjusted_x)))

    def fuseforward(self, x):
        adjusted_x = x * self.contrast_factor
        return self.act(self.conv(adjusted_x))
